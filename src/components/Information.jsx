...
export default function Information(props) {
    ...
    const [toLanguage, setToLanguage] = useState('Select languages')
    ...
    useEffect(() => {
      if(!worker.current){
        worker.current = new Worker(new URL('../utils/translate.worker.js',
          import.meta.url),
        {type: 'module'
      })
      }
    ...
    return (
        ...
        justify-center pb-20 max-w-prose w-full mx-auto'>
            ...
            whitespace-nowrap'>Your <span className='text-blue-400 bold'>
                Transcription</span></h1>

            <div className='grid grid-cols-2 mx-auto bg-white shadow rounded-full
            overflow-hidden items-center'>
                <button onClick={() => setTab('transcription')} 
                className={'px-4 duration-200 py-1 ' + 
                ...
                <div className='flex items-center gap-4 mx-auto text-base'>
                    <button onClick={handleCopy} title='copy' className='bg-white hover:text-blue-500 duration-200
                    text-blue-300 px-2 aspect-square grid place-items-center rounded'>
                        <i className='fa-solid fa-copy'></i>
                    </button>
                    <button onClick={handleDownload} title='download' className='bg-white hover:text-blue-500 duration-200
                    text-blue-300 px-2 aspect-square grid place-items-center rounded'>
